...
import { ImageGenerationResponse, usePostWithRetry } from "@/hooks/use-post-with-retry";
...
export default function Home() {
  ...
  const [selectedPreview, setSelectedPreview] = useState<ImageGenerationResponse | null> (null);
  const {history, addEntry} = useHistory();

  const handleGeneration = async(data: {prompt:string, style:string, imageDataUrl:string})=>{
    try{
  ...
  const handlePreviewSelection = useCallback((preview: ImageGenerationResponse) => {
    ...
    <div className="flex flex-col bg-background text-foreground sm:px-60">
      <div className="flex flex-1 overflow-hidden">
        <main className="flex mt-10 flex-1 overflow-hidden gap-10 flex-wrap">
          ...
          />
          {selectedPreview && <Preview result={selectedPreview} open={open} setOpen={setOpen}/>}
