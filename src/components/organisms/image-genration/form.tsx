...
import { FileUploader, FileUploaderRef } from "@/components/ui/molecules/file-uploader";

interface ImagePromptFormProps {
  onSubmit: (data: { imageDataUrl: string; prompt: string; style: string }) => void;
  onCancel?: () => void;
  ...
  return (
    <div className=" border border-border bg-card backdrop-blur-sm sm:w-[500px] rounded-md">
      <div className="p-4">
        ...
        {!uploadedImage && (
         <FileUploader ref={uploaderRef}  error={fileUploadError} handleFileSelect={handleFileSelect} handleDrop={handleDrop} handleDragOver={handleDragOver} />
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <div className="flex space-x-2 flex-col">
            ...
            <SelectBox options={STYLES} onValueChange={setStyle} value={style}  className="w-full" id="style" testId="style"/>
        ...
        {(uploadedImage || prompt) && (
          ...
          >
            ...
            <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium" data-testid="summary-image-name">
                          {uploadedImage ? uploadedImage.name : 'No image uploaded'}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {uploadedImage ? 'Ready for generation' : 'Upload an image to get started'}
                        </p>
                      </div>
                    </div>  
             
              <span data-testid="summary-style" className="bg-green-200 text-green-800 rounded-lg px-2 py-1 self-start">
                {STYLES.find((s) => s.value === style)?.label}
              </span>
              <span data-testid="summary-prompt" className="text-wrap">{prompt || ""}</span>
            </div>
          </div>
        )}
      </div>
