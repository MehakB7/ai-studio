...
describe('History Component', () => {
	it('should show correct date format', () => {
		...
		expect(screen.getByTestId('history-time-1').textContent).toMatch(/hour|minute|second/i);
		expect(screen.getByTestId('history-time-2').textContent).toMatch(/day/i);
	});

	it('should show image, title and style tag', () => {
		render(<History history={mockHistory} loadFromHistory={jest.fn()} />);
		expect(screen.getByTestId('history-image-1')).toHaveAttribute('src', mockHistory[0].imageUrl);
		expect(screen.getByTestId('history-image-2')).toHaveAttribute('src', mockHistory[1].imageUrl);
		expect(screen.getByTestId('history-prompt-1')).toHaveTextContent(mockHistory[0].prompt);
		expect(screen.getByTestId('history-prompt-2')).toHaveTextContent(mockHistory[1].prompt);
    ...
    it('should call loadFromHistory when image is clicked', () => {
        ...
        render(<History history={mockHistory} loadFromHistory={mockLoadFromHistory} />);
